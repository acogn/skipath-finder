...
def draw_curved_edge(pos, node1, node2, color, label, rad, ax):
    ...
    node_radius = np.sqrt(1500) / 2  # node_size is 1500
    ...
    curve = plt.matplotlib.patches.FancyArrowPatch(
        ...
        color=color,
        alpha=0.7,  # Slightly increased alpha for better visibility of thin lines
        linewidth=0.7,  # Made lines thinner
...
def visualize_ski_resort():
    ...
    # Add legend
    legend_elements = [
        plt.Line2D([0], [0], color='gray', label='Lifts', linewidth=2),
        plt.Line2D([0], [0], color='blue', label='Blue Slopes', linewidth=2),
        plt.Line2D([0], [0], color='red', label='Red Slopes', linewidth=2),
        plt.Line2D([0], [0], color='black', label='Black Slopes', linewidth=2),
    ]
    ...
    plt.title("Les Arcs Ski Resort Graph", pad=20, size=14)
    ax.margins(0.2)
    plt.axis('off')
    plt.tight_layout()
